...
// Depending on user logged in or not will display a menu
...
elseif (isset($_POST['username']))
{
	...
	// make copies of the data user entered and store in the variables
	...
	// sanitise is done to make sure there are no characters that are reserved in the data entered by the user for example quotation marks.
	...
	if ($errors == "")
	{
		...
		// as this query can return data we store it into variable $result
		$result = mysqli_query($connection, $query);

		// store number of rows returned into the variable $n
...
<form action="sign_in.php" method="post">
  ...
  Username: <input type="text" name="username" maxlength="16" placeholder="Username" value="$username" required> $username_val
  <br>
  Password: <input type="password" name="password" maxlength="16" placeholder="Password" value="$password" required> $password_val
  <br>
